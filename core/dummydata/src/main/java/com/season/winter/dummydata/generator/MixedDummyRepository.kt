...
class MixedDummyRepository {

    private var cachePhotoUrls: List<String>? = null
    ...
    suspend fun getRandomList(): List<MixedDummyData> = withContext(Dispatchers.IO) {
        ...
        cachePhotoUrls = getRandomPhoto((setRandomNumber + 5)) // 사진 중복률 줄이기 위해 5장 더 다양한 이미지 불러오기
        mutableListOf<MixedDummyData>().apply {
